...
## D9 — Dashboard album-card "..." menu (Edit / Delete)

The dashboard's album cards (`test`, `aaa`, etc. in the current screenshots) currently only have "Copy link" and "View album" — there's no way to rename an album, change its client name, set a cover photo, or delete it without opening the album first. Add a "..." (three-dot, vertical) icon button to each card — this is a **different** menu from the per-photo "..." menu already specified in `2026-07-09-photo-action-ui-design.md`; visually it can follow the same "opaque anchored dropdown, no list bullets" treatment specified there (D5's binding rules apply here too), but its contents are album-level, not photo-level:

- **Edit album** — opens a form (modal, matching the existing `CreateAlbumModal` pattern, is the natural fit) with the album's current name and client name pre-filled, plus a way to pick a cover photo from the album's existing photos (a small thumbnail picker — grid of the album's current photos, click to select one as cover; only shows/enables once the album has at least one photo, per `2026-07-09-album-editing-design.md`'s decision that cover photos are always chosen from existing photos, never uploaded separately). Submits via `PATCH /api/albums/[albumId]` (`docs/superpowers/plans/2026-07-09-album-editing.md`), sending only the fields that actually changed.
- **Delete album** — same action as D8, just also reachable from here for convenience; still requires confirmation.
- Once a cover photo is set, the dashboard card should display it (replacing whatever placeholder/first-photo the card currently shows, if any) — `album.coverPhoto` is available via the same query the dashboard already uses to list albums (add `coverPhoto: { select: { thumbnailUrl: true } }` to whatever `include`/`select` the albums-list query already has).

## D10 — Create-album form is missing the Drive-link field again

`docs/superpowers/plans/2026-07-09-drive-link-album-creation.md` is fully implemented on the backend (`POST /api/albums` accepts `driveLink` and imports existing photos) — but the current `CreateAlbumForm` (screenshotted during review) only has Album name / Client name / two toggles. The "Link ảnh Google Drive" field that appeared in an earlier iteration of this form was removed at some point (reasonably, at the time, since it wasn't wired to anything yet) and needs to come back now that there's a real route behind it. Required field, positioned first (matching the original ShotPik reference this whole redesign started from), sending its value as `driveLink` in the `POST /api/albums` body, and surfacing the response's `imported`/`skipped` counts to the photographer after creation succeeds.
