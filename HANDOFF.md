...
Run the test suite with `npx vitest run` (254 tests, all passing as of this update) and verify the production build with `npx next build`.
...
7. **Create album via Drive link** (`POST /api/albums` now accepts `{ name, clientName, driveLink }`) — parses the link, checks edit access, imports existing supported images (jpg/png/webp, skipping RAW/xmp/video), returns `{ imported, skipped }`. **The UI's `googleDriveLink` field is not yet wired to send `driveLink` in the request body — this is the next concrete UI task, see below.**
8. **Album deletion** — `DELETE /api/albums/[albumId]` (photographer/admin only, web-only delete, never touches Drive, cascades to Photo/Like/Comment) plus an automatic check on the album detail page that removes an album from the app if its Drive folder is confirmed gone (trashed or a real 404 — never on a transient error).
...
- **D6/D7/D8/D9/D10 in `docs/superpowers/specs/2026-07-09-ui-architecture-adr.md`**:
  - D6: the album-detail "Client Access" link needs to render as a full absolute URL with a one-click Copy button and a client-side QR code (`qrcode.react` or similar) — never shorten the actual `shareToken`, only the *displayed* text may be truncated.
  ...
  - D8: an "Edit album" button already exists on the album-detail header (good) — confirm it calls the real `PATCH /api/albums/[albumId]` (see item 9 above, there was a superseded draft). A "Delete album" button with a confirmation step is still needed (calling `DELETE /api/albums/[albumId]`), plus the "album no longer available" fallback state still needs restyling (currently plain unstyled markup from the backend plan).
  - **D9 (new)**: the dashboard's album cards need a "..." menu (separate from the per-photo one) offering "Edit album" (name/client/cover-photo form — a small picker grid of the album's own existing photos, since cover photos are always chosen from photos already in the album, never uploaded separately) and "Delete album". See D9 for full detail, including that the dashboard's album-list query needs `coverPhoto: { select: { thumbnailUrl: true } }` added so cards can actually display the chosen cover.
  - **D10 (new)**: the create-album form is still missing the "Link ảnh Google Drive" field despite `POST /api/albums` fully supporting `driveLink` now (item 7 above) — it was removed at some point when it had no backend, needs to come back.
...
- **Wire `CreateAlbumForm`'s `googleDriveLink` field to actually send `driveLink`** in the `POST /api/albums` body (see item 7 above) and surface the `imported`/`skipped` counts from the response.
