...
Run the test suite with `npx vitest run` (239 tests, all passing as of this update) and verify the production build with `npx next build`.
...
- **D6/D7/D8 in `docs/superpowers/specs/2026-07-09-ui-architecture-adr.md`**:
  ...
  - D7: the header banner needs a rewrite — "by {photographerName}" instead of the raw database ID; "Album: {name}" and "Client: {clientName}" as two separate labeled lines instead of one concatenated line; drop the location row entirely; photo count simplified to icon+number only.
  - D8: add a "Delete album" button (with a confirmation step — irreversible) calling the now-real `DELETE /api/albums/[albumId]`, plus render the "album no longer available" fallback state nicely (currently it's a plain unstyled message from the backend plan, needs restyling to match the rest of the app).
